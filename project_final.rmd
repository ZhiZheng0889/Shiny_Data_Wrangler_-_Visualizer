...
histogramUI <- function(id) {
    ns <- NS(id)
    tagList(
        selectInput(ns("selectedColumnHist"), "Select Column for Histogram:", choices = NULL),
...
histogramServer <- function(id, data) {
    moduleServer(id, function(input, output, session) {
        ...
        # Observe changes in the dataset and update the column choices
        observe({
            updateSelectInput(session, ns("selectedColumnHist"), choices = names(data()))
        })

        output$histPlot <- renderPlot({
            req(input$selectedColumnHist)
            hist(
                data()[[input$selectedColumnHist]],
                main = "Histogram",
                col = "lightblue",
                border = "black"
            )
...
boxPlotUI <- function(id) {
    ns <- NS(id)
    tagList(
        selectInput(ns("selectedColumnBox"), "Select Column for Box Plot:", choices = NULL),
...
boxPlotServer <- function(id, data) {
    moduleServer(id, function(input, output, session) {
        ...
        # Observe changes in the dataset and update the column choices
        observe({
            updateSelectInput(session, ns("selectedColumnBox"), choices = names(data()))
        })

        output$boxPlot <- renderPlot({
            req(input$selectedColumnBox)
            boxplot(
                data()[[input$selectedColumnBox]],
                main = "Box Plot",
                col = "lightgreen",
                border = "black"
            )
            ...
            tabsetPanel(
                ...
                tabPanel("Data Visualization",
                    tabsetPanel(
                        ...
                        tabPanel("Box Plot", boxPlotUI("boxPlotModule")),
...
server <- function(input, output, session) {
    ...
    # Call the boxplot chart module
    boxPlotServer("boxPlotModule", selectedData)
